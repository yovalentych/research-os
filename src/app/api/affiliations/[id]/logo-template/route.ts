...
export async function POST(
  ...
  try {
    ...
    const key = `affiliations/${id}/template-${Date.now()}-${sanitizeFileName(
      file.name
    )}`;
    ...
    await r2Client.send(
      new PutObjectCommand({
        ...
        Metadata: { affiliationId: id, template: "true" },
      })
    );

    const updates = { emblemTemplateStorage: { bucket: R2_BUCKET, key } };
