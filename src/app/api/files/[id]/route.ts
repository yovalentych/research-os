...
export async function PATCH(
  ...
  const previous = await FileItem.findById(fileId).lean();
  ...
  if (Array.isArray(body?.folderIds)) {
    updates.folderIds = body.folderIds.map((id: string) => new mongoose.Types.ObjectId(id));
  }
  ...
  const updated = await FileItem.findByIdAndUpdate(fileId, updateDoc, {
    new: true,
  }).lean();
...
export async function DELETE(
  ...
  const updated = await FileItem.findByIdAndUpdate(
    fileId,
    { archivedAt: new Date() },
    { new: true }
  ).lean();
