...
export async function PATCH(
  ...
  const materialId = new mongoose.Types.ObjectId(materialIdParam);
  const previous = await ProjectMaterial.findById(materialId).lean();
  if (!previous) {
  ...
  const updated = await ProjectMaterial.findByIdAndUpdate(
    materialId,
    updateDoc,
    {
      new: true,
